import DVector, { BUILD_VEC } from "../main/DVector";

test("test creation", () => {
  const expectedArray = testArray(1, 1, 1);
  const u = new DVector([1, 1, 1]);
  const v = DVector.of(1, 1, 1);
  const w = DVector.fromArray(expectedArray);
...
test("test size", () => {
  const v = DVector.of(1, 2, 3);
  expect(v.n).toBe(3);
  expect(v.size()).toBe(3);
  expect(v.shape()).toStrictEqual([3]);
});
test("test map", () => {
  const u = DVector.of(1, 1, 1).map(x => 2 * x);
  expect(u.toArray()).toStrictEqual(testArray(2, 2, 2));
});
test("test fold", () => {
  const sum = DVector.of(1, 1, 1).reduce((e, x) => e + x, 0);
  expect(sum).toStrictEqual(3);
  const sum2 = DVector.of(1, 1, 1).fold((e, x) => e + x, 0);
  expect(sum2).toStrictEqual(3);
});
test("test operation", () => {
  const u = DVector.of(1, 1, 1);
  const v = u.op(u, (a, b) => a + b);
  expect(v.toArray()).toStrictEqual(testArray(2, 2, 2));
});
test("test add", () => {
  const u = DVector.of(1, 2);
  const v = DVector.of(1, 1);
...
test("test equals", () => {
  const u = DVector.of(1, 1, 1, 1);
  expect(u.equals(u)).toBe(true);
});
test("test take", () => {
  const u = DVector.of(1, 2, 3, 4);
...
test("get i", () => {
  expect(DVector.e(10)(7).get(7)).toBe(1);
  expect(DVector.e(10)(7).get(8)).toBe(0);
});

test("copy", () => {
  expect(DVector.of(1, 2, 3).copy().toArray()).toStrictEqual(
    testArray(1, 2, 3)
  );
});
...
test("performance test", () => {
  ...
  const u = DVector.e(n)(0);
  const v = DVector.e(n)(1);
